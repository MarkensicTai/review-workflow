---
description: 先确认 Review Brief，再并行且独立执行 code-review 和 trace-review
---
...
2. Parallel Review：Review Brief 确认后，基于同一份已确认 Review Brief 和当前代码变更，并行执行：
   - `code-review`：执行合并前代码质量审查。
   - `trace-review`：执行上线后可追踪性审查。
