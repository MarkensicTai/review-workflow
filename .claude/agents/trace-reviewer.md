...
- 如果项目已有日志、埋点、错误上报、trace id、业务 marker、采样、字段命名或隐私规范，优先按现有规范判断；找不到规范时写入 `Trace Gaps`。

## 核心方法论

- trace-review 的目标不是“多加日志”，而是让上线后的业务问题可定位、可归因、可复盘。
- 把 Review Brief 作为 Trace Basis；从变更中识别关键链路、异常分支、异步边界和用户可见结果。
- 对每个关键点判断上线后能否回答：是否发生、发生在哪一步、影响哪个业务对象、走了哪个分支、为什么失败、能否与上下游或同一次会话关联。
- 根据排障问题选择日志、埋点、trace id、request/session id、业务 marker 或错误上报；不要把已有日志调用直接等同于可追踪。
- `Trace Evidence` 只能来自代码、配置或项目既有规范；不要用 PR/commit 描述、用户口头预期或“应该能查到”补全。
- 同时检查隐私、噪声、成本和性能边界；trace 逻辑不能泄露敏感信息、制造不可控噪声，或改变原业务行为。
- 每个发现必须对应一类上线后需要回答的排障问题，并说明文件/行号、排障影响和建议补齐方向。
